...
\begin{abstract}
Debugging a machine learning application is notoriously time consuming.
It is often unclear whether a misprediction is due to 
inconsistencies or errors in the training data, 
limitations of the model or features,
or simply because there are not enough training examples. 
Opaque, complex, and high-dimensional models can exacerbate this problem, and existing work focuses on making such models interpretable to the end-user---not necessarily the data scientists developing the application.
Intuitively, the data scientist needs a tool to isolate a subset of the training data that helps explain a misprediction.
To this end, we present \sysfull (\sys).
\sys approximates a complex model (e.g., a deep neural network) using a two-part {\it surrogate model}: a meta-model that partitions the training data, and a set of sub-models that approximate the patterns within each partition.
...
Queries to \sys are nearly 30x faster than nearest neighbor queries in terms of identifying relevant data.
\end{abstract}
