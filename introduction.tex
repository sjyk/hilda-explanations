...
Debugging processes are notoriously long and time-consuming~\cite{sculley2014machine, aidebugging}.
...
Debugging machine learning models is challenging because it is often unclear whether a misprediction is due to inconsistencies or errors in the training data, 
...
Developers must be able to understand why a model made a certain (mis)prediction to effectively design and debug their machine learning applications.
...
Also, identifying training data can help developers differentiate between errors due to the model and errors due to training data.


%These approximations are effective because they can explain a prediction based on features of the input data (e.g., linear weights), but are these approximations also useful for debugging?
...
Applying the trained surrogate model is very simple---if a test point is mispredicted, then we can identify the partition it belongs to and investigates that subset of the training data and its associated sub-model.
