...
\ewu{Provide intuition on why the decision tree + models is a good idea.}
Intuitively, a subset of the training data is useful for debugging a mispredicted test point if a model built on that subset of data is \ewu{XXX}.  Subsets of training data are most easily explained as rules (predicates).  Given this intuition, our goal is to construct a {\it surrogate} model that parititons the training dataset by axis -align splits (decision tree) and fits a high quality model to each parititon in such a way that both approximates the DNN model and \ewu{XXX}

\ewu{At this point, I think things are set up to actually introduce the notion of debuggability}
